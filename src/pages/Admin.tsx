...
import { LogOut, RefreshCw, Search, ShieldCheck, Key, ChevronLeft, ChevronRight } from "lucide-react";
...
export default function Admin() {
  ...
  const checkAuth = async () => {
    try {
      const res = await fetch("https://www.internship.unicodetechnolab.com/API/admin_auth.php", {
        method: "POST",
        credentials: "include",
  ...
  const handleLogout = async () => {
    try {
      await fetch("https://www.internship.unicodetechnolab.com/API/admin_auth.php", {
        method: "POST",
        credentials: "include",
        headers: { "Content-Type": "application/json" },
  ...
  const fetchInterns = async () => {
    setLoadingData(true);
    try {
      const res = await fetch("https://www.internship.unicodetechnolab.com/API/get_interns.php", {
        credentials: "include",
      });
      const data = await res.json();
      if (data.success) {
  ...
  const filteredInterns = interns.filter((intern) => {
    ...
    const matchType = filterType === "all" || intern.internship_type === filterType;

    return matchQuery && matchType;
  });

  const uniqueTypes = Array.from(new Set(interns.map((i) => i.internship_type)));
      ...
      <div className="min-h-screen flex items-center justify-center bg-bg p-4 pt-32 pb-24">
        <div className="bg-white border border-border rounded-sm shadow-sm p-8 max-w-md w-full">
          <div className="text-center mb-8">
            ...
            <p className="text-sm text-muted mt-2">Secure access for administrators</p>
          </div>
          ...
          {authStep === "email" ? (
            <form onSubmit={handleSendOtp} className="space-y-4">
              ...
              >
                {authLoading ? "Sending..." : "Send Verification Code to Admin Email"}
              </button>
            </form>
          ) : (
    ...
    <div className="min-h-screen bg-bg pt-32 pb-24">
      <div className="max-w-7xl mx-auto container-pad">
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-4 mb-8">
          <div>
            <h1 className="text-3xl font-black text-ink mb-2">Registered Interns</h1>
            <p className="text-sm text-muted">Manage and view all internship applications</p>
          </div>
              ...
              <tbody className="divide-y divide-border">
                {loadingData ? (
                  ...
                  paginatedInterns.map((intern, index) => (
                    <tr key={intern.id} className="hover:bg-surface/50 transition-colors">
                      ...
                      <td className="px-4 py-3">
                        <div className="flex flex-col gap-0.5">
                          <a href={`mailto:${intern.email}`} className="text-brand hover:underline">
                            {intern.email}
                          </a>
                          <span className="text-muted text-xs">{intern.phone}</span>
                        </div>
                      </td>
                      <td className="px-4 py-3">
                      ...
                      <td className="px-4 py-3">
                        <div className="flex flex-col gap-0.5">
                          <span className="text-ink truncate max-w-[200px]" title={intern.college}>
                            {intern.college}
                          </span>
                          <span className="text-muted text-xs">{intern.city}</span>
                        </div>
                      </td>
            ...
            <div className="border-t border-border px-4 py-4 bg-surface/70 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
              <div className="flex items-center gap-2">
                <p className="text-xs font-medium text-muted">
                  Showing {startIndex + 1}-{endIndex} of {filteredInterns.length} records
                </p>
              </div>
              ...
              <div className="flex items-center gap-2">
                ...
                <p className="text-xs font-semibold text-ink min-w-[3rem] text-center">
